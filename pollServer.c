...
void *recvFromClient(void *arg){
    ...
    Preqests reqest = &((Preqests)arg)[0];
    int new_fd = reqest->id_file;
    char buf[1024];
    while(1){
        ...
        if (b <= 0){
            ...
            for (int i = 0; i < fd_count; i++){
                ...
                if (id_client != listener && id_client != new_fd){
                    send(id_client, buf, b, 0);
                }
            }
            bzero(buf, 1024);
        }
            ...
            if (pfds[i].revents & POLLIN) { // We got one!!
                ...
                if (pfds[i].fd == listener) {
                    ...
                    if (newfd == -1) {
                        ...
                        InstallHandler(reactor, &recvFromClient, newfd);
                    }
                } else {
